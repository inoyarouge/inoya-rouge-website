...
export default function Navbar() {
  return (
    ...
    >
      <div className="site-container px-6 md:px-12 w-full h-full flex items-center">
        {/* Desktop Layout */}
            ...
            <div key={link.href} className={`group/navitem h-full flex items-center${link.textDropdown ? ' relative' : ''}`}>
              ...
              {link.dropdown && (
                <div className="hidden md:block absolute top-full left-0 w-full bg-[#fff8f6] shadow-xl border-t border-black/5
                  ...
                  <div className="site-container px-6 md:px-12 py-10 flex justify-center gap-6 md:gap-12">
                    {link.dropdown.map((item, i) => (
                      <Link href={item.href} key={item.name} style={{ animationDelay: `${i * 100}ms` }} className="mega-card group/dropitem relative w-[280px] aspect-square rounded-none overflow-hidden shadow-md hover:shadow-xl transition-all duration-500">
