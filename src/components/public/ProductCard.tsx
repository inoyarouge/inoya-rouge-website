  ...
  if (variant === 'shop') {
    return (
      <div className="group flex flex-col relative w-full bg-transparent h-full">
        <Link href={`/shop/${product.slug}`} className="block focus:outline-none focus-visible:ring-2 focus-visible:ring-burgundy focus-visible:ring-offset-2 rounded-md h-full flex flex-col">
          ...
          <div className="flex flex-col mt-4 px-1 flex-grow">
            {/* Sub-category / Type */}
            ...
            {/* Swatches Section - pushed down using mt-auto */}
            <div className="mt-auto pt-6 pb-4">
              <div className="flex items-center gap-2 pl-0.5">
                {activeVariants.slice(0, 4).map((v, idx) => (
                  <div
                    key={v.id}
                    ...
                    className={`w-[20px] h-[20px] rounded-full border ${currentVariant?.id === v.id ? 'border-burgundy' : 'border-burgundy/20'} p-[1px] hover:border-burgundy cursor-pointer transition-colors`}
                  >
                    <div
        ...
        <div className="mt-0">
          <Link
            href={`/shop/${product.slug}?variant=${currentVariant?.id || ''}`}
          ...
          >
            ...
            <span className="absolute inset-0 w-full h-full bg-[#7D0000] -translate-x-full group-hover/btn:translate-x-0 transition-transform duration-500 ease-[cubic-bezier(0.25,0.46,0.45,0.94)]" />
